...
def extract_features(X, colorspace, visualize=False):
    ...
    for img in X:
        ...
        hog_images = []
        for i in range(3):
            features, hog_image = hog(img[...,i], orientations=HOG_ANGLE_BINS, pixels_per_cell=HOG_CELL_SIZE, cells_per_block=HOG_CELLS_PER_BLOCK, block_norm='L2-Hys', visualise=True, transform_sqrt=False, feature_vector=True)
...
def train(X, y):
    ...
    algo = svm.SVC()
    classifier = grid_search.GridSearchCV(svr, parameters)
...
def main():
    ...
    X = extract_features(images, args.colorspace, args.visualize)
